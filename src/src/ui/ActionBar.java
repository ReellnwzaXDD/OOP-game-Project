...
public class ActionBar extends Bar {
	...
	public void draw(Graphics g) {
		...
		// Background
		g.setColor(new Color(235, 171, 89)); //220 123 15
	...
	public void drawLives(Graphics g) {
		InputStream is = getClass().getResourceAsStream("/res/heart.png");
		try {
			img = ImageIO.read(is);
		} catch (IOException e) {
			e.printStackTrace();
		}
		g.drawImage(img, 105, 730, null);
		g.setColor(Color.black);
		g.drawString("Lives: " + lives, 150, 750);
	}
	...
	private void drawGoldAmount(Graphics g) {
		InputStream is = getClass().getResourceAsStream("/res/gold.png");
		try {
			img = ImageIO.read(is);
		} catch (IOException e) {
			e.printStackTrace();
		}
		g.drawImage(img, 110, 705, null);
		g.drawString("Gold: " + gold + "g", 150, 724);
